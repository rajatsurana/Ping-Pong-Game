...
//	}
	...
	public CustomBoard(int max, int peers,network net,String pos) {
		this.position = pos;
	...
	private void initBoard() {
		...
		paddle = new Paddle(net.getName());
		System.out.println(network.listofpeers.size() +" :noofpeers"+"\n"+
						net.getName()+" "+net.listofpeers.get(0).connectedto());
		paddle2 = new Paddle(network.listofpeers.get(0).connectedto());//network.peermanage.listofpeers.get(0).mydata.name
		ball = new Ball();
	...
	private void doDrawing(Graphics g) {
		...
		drawPaddels(g);
		/*
		 * paddle2 player if (paddle2Expand == false) {
		 * g.setColor(Color.yellow); g.fillRect(paddle2.getX(), paddle2.getY(),
		 * 150, 5); } else { g.setColor(Color.magenta);
	...
	/*
	 * private void updatepaddle2() { if (ball.getVX() > 0) { if (ball.getX() +
	 * 25 > paddle2.getX() + 75) paddle2.moveRight(); } else { if (ball.getX() +
	 * 25 < paddle2.getX() + 75) paddle2.moveLeft(); } }
	 */
	...
	private void checkCollisions() {
		...
		if (ball2_visi) {
			 ...
			 * " " + (ball.getY() - paddle2.getY())); int velX = ball2.getVX();
			 * int velY = ball2.getVY(); int temp = -velY; ball2.setVel(velX,
			 * temp);
			 * 
			 * }
			 */
				...
				if (ballRect.intersects(powerRect)) {
					...
					if (tempid == 0) {
						if (hitby.equals("Player")) {
							ball = new Ball();
							if(position.equals("DOWN")){
								net.broadcast("Ball " + ball.getX() + " "
										+ ball.getY() + " " +ball.getVX() + " "+ ball.getVY() + " " + paddle.getName());
		...
		public void keyPressed(KeyEvent e) {
			...
			if (position.equals("UP") || position.equals("DOWN")) {
				...
				if (e.getKeyCode() == KeyEvent.VK_LEFT) {
					net.broadcast("Left " + paddle.getX() + " "
							+ paddle.getDx() + " " + paddle.getName());
				} else if (e.getKeyCode() == KeyEvent.VK_RIGHT) {
					net.broadcast("Right " + paddle.getX() + " "
							+ paddle.getDx() + " " + paddle.getName());
				}
			}
			if (position.equals("UP") || position.equals("DOWN")) {
				...
				if (e.getKeyCode() == KeyEvent.VK_A) {
					...
					net.broadcast("Right " + paddle.getX() + " "+ paddle.getDx() + " " + paddle.getName());// ("R2 "+paddle2.getX()+" "+paddle2.getDx());
				}
			}
